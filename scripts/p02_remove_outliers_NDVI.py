...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 18 10:21:11 2026
...
# -*- coding: utf-8 -*-
"""
Remove NDVI outliers from the new per-polygon CSV output.
...
# Rolling-window dip removal parameters
dip_window_days = 21
dip_k = 3.0
dip_min_drop = 0.10
...
# Optional gentle IQR guardrail after smoothing
...
def ultrasmooth_optionC_with_qc(df: pd.DataFrame,
    ...
    # Remove dips and interpolate again
    daily.loc[dip_mask, ndvi_col] = np.nan
    daily[ndvi_col] = daily[ndvi_col].interpolate(method="time", limit_direction="both")

    # Final robust smoothing
    ndvi_clean = (
        daily[ndvi_col]
...
for keys, g in df.groupby(group_cols, dropna=False):
    ...
    if len(g) < 3:
        ...
        out_csv = os.path.join(output_folder, f"zone_{int(zone_id):02d}_{safe_name}_part_{int(part_id)}_cleaned_C_QC.csv")
    ...
    # -------------------------------------------------
    # STEP 3: Ultrasmooth with rolling dip removal + QC
    # -------------------------------------------------
    ...
    sampled = sampled[["zone_id", "source_name", "part_id", date_col,
                       "NDVI_raw_first_per_date", "NDVI_original_obs",
                       "NDVI_clean", "QC_flag"]].sort_values(date_col)
...
if all_cleaned:
    ...
    combined_csv = os.path.join(output_folder, "all_polygons_NDVI_cleaned_C_QC.csv")
